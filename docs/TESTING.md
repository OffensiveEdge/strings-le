# String-LE Testing Guidelines

This document outlines testing practices and policies for String-LE development.
...
```typescript
// ✅ Good
it('should extract user-visible strings from JSON files', () => {
  // ...
});
...
```typescript
// ✅ Good - separate tests
it('should extract strings from values', () => { /* ... */ });
it('should exclude technical identifiers', () => { /* ... */ });

// ❌ Bad - multiple concerns
it('should extract strings and exclude IDs', () => { /* ... */ });
```

### 3. Use Arrange-Act-Assert Pattern

```typescript
it('should extract strings from JSON', () => {
  // Arrange
  const json = '{"message": "Hello, world!"}';
  
  // Act
  const result = extractStrings(json);
  
  // Assert
  expect(result.strings).toContainEqual({
    value: 'Hello, world!',
    type: 'string',
  });
});
