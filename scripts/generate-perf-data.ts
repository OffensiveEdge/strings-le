#!/usr/bin/env bun
/**
 * Complete Performance Pipeline for string-le
 *
...
const FILE_SPECS: readonly FileSpec[] = Object.freeze([
  ...
  { name: "5k.env", targetSizeMB: 0.01, format: "env" },
...
function generateEnvData(targetBytes: number): string {
  let env = "";
  let currentSize = 0;

  while (currentSize < targetBytes) {
    const varName = `VAR_${Math.random()
      .toString(36)
      .substring(2, 10)
      .toUpperCase()}`;
    const varValue = `"value_${Math.random()
      .toString(36)
      .substring(7)}_${Math.floor(Math.random() * 1000)}"`;
...
function generateData(format: string, targetBytes: number): string {
  switch (format) {
    ...
    case "env":
      return generateEnvData(targetBytes);
    default:
...
function extractPerformanceReport(benchmarkOutput: string): string {
  const startMarker = "# Stringle Performance Test Results";
...
function main() {
  console.log("🚀 Complete Performance Pipeline for string-le\n");
