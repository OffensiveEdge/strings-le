...
const PERFORMANCE_FILES = Object.freeze([
	...
	// Removed unreliable format tests
] as const);
...
function runSinglePerformanceTest(
	...
	const throughputLinesPerSecond = Math.round(
		(lineCount / extractionTimeMs) * 1000,
	);
...
function runPerformanceTests(): PerformanceResult {
	...
	for (const { name, format } of PERFORMANCE_FILES) {
		...
		try {
			...
			console.log(
				`  ✓ Throughput: ${metric.throughputLinesPerSecond.toLocaleString()} lines/sec`,
			);
			console.log('');
		} catch (error) {
...
function formatPerformanceReport(result: PerformanceResult): string {
	...
	for (const metric of metrics) {
		...
		} | ${metric.extractedCount.toLocaleString()} | ${metric.throughputLinesPerSecond.toLocaleString()} | ${
