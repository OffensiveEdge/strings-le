...
describe('extractStrings', () => {
	...
	it('should JSON with nested objects', () => {
		...
		expect(result).toEqual(['John Doe', 'john@example.com', 'dark', 'en', 'strict'])
	})
	...
	it('should JSON with arrays', () => {
		...
		const result = extractStrings(json, 'json')
		expect(result).toEqual(['urgent', 'important', 'review', 'Alice', 'admin', 'Bob', 'user'])
	})
  ...
  Jane,jane@example.com`
		...
		const result = extractStrings(csv, 'csv')
		expect(result).toEqual(['name', 'email', 'John', 'john@example.com', 'Jane', 'jane@example.com'])
	})
  ...
  SPACE=" "`
    ...
    Empty "" and 'single' quotes`
		...
		expect(result).toEqual(['world', 'universe', 'backticks', "nested 'quotes'", 'single'])
	})
