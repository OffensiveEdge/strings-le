...
DB_PASSWORD=secret123`
		...
		expect(result).toEqual(['localhost', '5432', 'myapp', 'admin', 'secret123'])
	})
...
API_KEY=secret123`
		...
		const result = extractDotenv(dotenv)
		expect(result).toEqual(['localhost', '5432', 'https://api.example.com', 'secret123'])
	})
...
REGEX="^[a-zA-Z0-9]+$"`
		...
		const result = extractDotenv(dotenv)
		expect(result).toEqual(['pass@word#123', 'https://api.example.com/v1/endpoint?param=value', '^[a-zA-Z0-9]+$'])
	})
