...
async function readDirFiles(dir: string): Promise<readonly string[]> {
	...
	return entries
		.filter(
			(name) => !name.endsWith('.expected.txt') && !name.endsWith('.meta.json'),
		)
		.sort();
}
...
async function readCsvMetaIfAny(
	filePath: string,
): Promise<CsvMeta | undefined> {
...
async function readExpectedLines(
	filePath: string,
...
async function writeExpectedLines(
	filePath: string,
...
function dedupeAndSort(values: readonly string[]): readonly string[] {
	// Filter out empty strings, then dedupe and sort
	const filtered = values.filter((s) => s.trim().length > 0);
...
async function runCase(filePath: string, update: boolean): Promise<void> {
	...
	assert.ok(
		expected,
		`Missing expected file for ${path.basename(filePath)} — run npm run test:update`,
	);
	try {
...
describe('integration tests', () => {
	it('should match expected outputs for data-driven fixtures', async () => {
		await main();
	});
});
