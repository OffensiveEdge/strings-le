...
describe('extract command', () => {
	...
	it('registers extract command', () => {
		...
		expect(commands.registerCommand).toHaveBeenCalledWith('string-le.extractStrings', expect.any(Function))
	})
	...
	it('shows warning when no active editor', async () => {
		...
		expect(mockNotifier.error).toHaveBeenCalledWith(expect.stringContaining('No active editor'))
	})
	...
	it('extracts strings from JSON file', async () => {
		;(window.activeTextEditor as any) = {
			document: {
				getText: vi.fn().mockReturnValue('{"key": "value", "title": "Hello World"}'),
				languageId: 'json',
				uri: { fsPath: '/test.json' },
				fileName: '/test.json',
			},
