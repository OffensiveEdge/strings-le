...
async function validateAndPrepareExtraction(
	...
	try {
		...
		deps.notifier.error(localize('runtime.message.error.document-invalid', 'Document is no longer valid'))
		return null
	}
}
		...
		for (const idx of targetIndexes) {
			...
			try {
				...
				if (error instanceof Error) {
					deps.notifier.error(
						localize('runtime.message.error.column-stream', 'Column {0} streaming failed: {1}', idx, error.message),
					)
				}
...
async function handleCsvStreamingExtraction(
	...
	try {
		...
		if (error instanceof Error) {
			deps.notifier.error(localize('runtime.message.error.csv-streaming', 'CSV streaming failed: {0}', error.message))
		} else {
			deps.notifier.error(
				localize('runtime.message.error.csv-streaming-unknown', 'CSV streaming failed with unknown error'),
			)
		}
