...
async function trimWhitespace(): Promise<void> {
	...
	const success = await processAndOutput(editor, trimmedText);

	if (success) {
		vscode.window.showInformationMessage(
			localize('runtime.info.trimmed', 'Trimmed whitespace from selection.'),
		);
	}
}

export function registerTrimWhitespaceCommand(
	context: vscode.ExtensionContext,
): void {
	context.subscriptions.push(
		vscode.commands.registerCommand(
			'string-le.postProcess.trim',
			trimWhitespace,
		),
	);
}
